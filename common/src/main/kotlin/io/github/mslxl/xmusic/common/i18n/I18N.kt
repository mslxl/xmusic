...
object I18NStorage {
    ...
    fun insert(src: MusicSource) {
        val textPair = if (src.i18n.containsKey(local)) {
            logger.info("install i18n text $local from ${src.name}")
            src.i18n[lang]!!.invoke()
        } else {
            src.i18n.keys.find { getLang(it) == lang }?.let {
                logger.info("install i18n text $lang from ${src.name}")
                src.i18n[it]!!.invoke()
            }
        }
        if (textPair == null) {
            logger.info("could find i18n text in source ${src.name}")
            return
        }
...
inline fun I18NKey.i18n(src: SourceID, vararg format: String) = I18NStorage.getString(src, this, *format)
